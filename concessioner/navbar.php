  ...
  <nav class="navbar navbar-dark bg-maroon fixed-top">
    ...
    <ul class="dropdown-menu">
      <li><?php 
                ...
                $sql = "SELECT * FROM tbl_concessioner WHERE concessionerID='{$_SESSION['concessionerID']}'";         
                $result = mysqli_query($conn, $sql);
             while($row = mysqli_fetch_assoc($result)){
               echo '<h6 class="ps-3">'.$row['business_name'].'</h6>';
               echo '<p class="ps-3 text-secondary">'.$row['firstName'] .' '. $row['lastName'].'</p>';
